...
const putRequires = async (req, res) => {
    const {id,state,method,total_cost,discount} = req.body
    ...
    if(state === "cancelado"){
        ...
        const [result] = await db.query("UPDATE required SET state= ?, method= ? , total_cost= ? ,discount= ? WHERE id_required = ?",[state,method,total_cost,discount,id])
        return res.status(200).json(result)
    }
