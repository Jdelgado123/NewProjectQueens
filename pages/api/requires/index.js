...
const putRequires = async (req, res) => {
    const {id,state,method,total_cost} = req.body
    ...
    if(state === "cancelado"){
        ...
        const [result] = await db.query("UPDATE required SET state= ?, method= ? , total_cost= ? WHERE id_required = ?",[state,method,total_cost,id])
        return res.status(200).json(result)
    }
