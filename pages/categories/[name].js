...
const categories = ({ products }) => {
  ...
  const togleModalImagesx = () => {
    document.querySelector('#modalImages').classList.toggle('hidden')
    document.querySelector('#sss').classList.toggle('showImage')
  }

  return (
    <Layout>
      <div className='grid md:px-10 gap-1 md:gap-6 grid-cols-3 md:grid-cols-4'>
        ...
        <div id='modalImages' className='hidden fixed w-full text-center h-full top-0 right-0 left-0 z-40 md:inset-0 h-modal md:h-full place-content-center min-h-screen grid transform'>
          <div className='animate-bounce-new fixed inset-0 bg-gray-700 bg-opacity-70' onClick={() => togleModalImagesx()} />
          ...
          <Image id='sss' className='animate-img' src={src} width={375} height={325} alt="........" />
