  ...
  return (
    <div className='grid gap-6 grid-cols-1 md:grid-cols-3 items-center justify-between'>
      {products.map((product, index) => (
        <div key={index} className="border border-solid border-black max-w-sm bg-white rounded-lg shadow-xl">
          <div className='items-center content-center justify-center object-center'>
          ...
          <div className="items-center grid grid-flow-col">
            <div className='content-center text-center justify-center'>
              <span className="text-3xl font-bold text-red-600/100 pb-2 pl-10">{product.currency=="USD"?"$":"S/"}<span className='text-4xl font-bold  text-gray-900 dark:text-black pb-2'>{product.price}</span></span>
            </div>
            <div className='cart-icon content-end flex justify-end pr-3'>
              <Link href={"/succes"}>
                <a href="#" className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"><AiOutlineShoppingCart></AiOutlineShoppingCart></a>
              </Link>
            </div>
          </div>
        </div>
